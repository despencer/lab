...
{
  gzlog << "Connection::Connection started, this=" << this << std::endl;
  this->isOpen = false;
  ...
  gzlog << "Connection::Connection finished\n";
}
...
{
  gzlog << "Connection::Connect started, host='" << _host << "', port=" << _port << ", this = " << this << "\n";
...
{
  gzlog << "Connection::Listen, port " << port << ", this = " << this << "\n";
...
void Connection::OnAccept(const boost::system::error_code &e)
{
  ...
  if (!e)
  {
    ...
    {
      gzlog << "Connection::OnAccept, this = " << this << ", prepare to call callback\n";
      this->acceptCB(this->acceptConn);
    }

    // First start a new acceptor
    this->acceptConn = ConnectionPtr(new Connection());
    gzlog << "Connection::OnAccept, this = " << this <<", accept connection = " << this->acceptConn << "\n";
  ...
  gzlog << "Connection::OnAccept returns, this" << this << "\n";
}
...
{
  gzlog << "Connection::EnqueueMsg started, force=" << _force << ", this=" << this << "\n";
  ...
  gzlog << "Connection::EnqueueMsg finished, this=" << this << "\n";
}

/////////////////////////////////////////////////
void Connection::ProcessWriteQueue(bool _blocking)
{
  gzlog << "Connection::ProcessWriteQueue started, blocking=" << _blocking << ", this=" << this << "\n";
  boost::recursive_mutex::scoped_lock lock(this->writeMutex);
  ...
  if (!_blocking)
  {
    gzlog << "Connection::ProcessWriteQueue, write to socket async, this=" << this << "\n";
  ...
  gzlog << "Connection::ProcessWriteQueue finished, this=" << this << "\n";
}
...
{
  gzlog << "Connection::Shutdown started, this=" << this << "\n";
  if (!this->socket)
    return;
...
{
  gzlog << "Connection::Close called, this = " << this << "\n";
...
{
  gzlog << "Connection::Cancel called, this = " << this << "\n";
  if (this->acceptor)
  {
    try
    {
...
{
  gzlog << "Connection::Read started, this=" << this << "\n";
  bool result = false;
  ...
  gzlog << "Connection::Read returns, this=" << this << "\n";
  return result;
}
