...
{
  gzlog << "ConnectionManager::ConnectionManager started\n";
  this->tmpIndex = 0;
  ...
  gzlog << "ConnectionManager::ConnectionManager finished\n";
}
...
{
  gzlog << "ConnectionManager::Init started\n";
  this->stop = false;
  this->masterConn.reset(new Connection());
  this->serverConn.reset(new Connection());

  gzlog << "ConnectionManager::Init, masterConn=" << this->masterConn << ", serverConn=" << this->serverConn << "\n";
  ...
  if (packet.type() == "topic_namepaces_init")
  {
    ...
    gzlog << "ConnectionManager::Init " << result.data_size() << " namespaces received\n";
  ...
  gzlog << "ConnectionManager::Init returns, " << this->namespaces.size() << " namespaces\n";
  return true;
}
...
{
  gzlog << "ConnectionManager::Fini started\n";
  if (!this->initialized)
    return;
  ...
  this->initialized = false;
  gzlog << "ConnectionManager::Fini stopped\n";
}
...
{
  gzlog << "ConnectionManager::RunUpdate started\n";
  ...
  gzlog << "ConnectionManager::RunUpdate, msize = " << msize << "\n";
  while (msize > 0)
  {
  ...
  gzlog << "ConnectionManager::RunUpdate, master finished\n";
  if (this->masterConn)
    this->masterConn->ProcessWriteQueue();
  ...
  gzlog << "ConnectionManager::RunUpdate finishes\n";
}

//////////////////////////////////////////////////
void ConnectionManager::Run()
{
  gzlog << "ConnectionManager::Run started\n";
  boost::mutex::scoped_lock lock(this->updateMutex);
  ...
  this->masterConn->Shutdown();
  gzlog << "ConnectionManager::Run stopped\n";
}
...
{
  gzlog << "ConnectionManager::OnMasterRead started\n";
  using namespace boost::placeholders;
  ...
  this->TriggerUpdate();
  gzlog << "ConnectionManager::OnMasterRead finished\n";
}

/////////////////////////////////////////////////
void ConnectionManager::ProcessMessage(const std::string &_data)
{
  gzlog << "ConnectionManager::ProcessMessage started\n";
  msgs::Packet packet;
  packet.ParseFromString(_data);

  gzlog << "ConnectionManager::ProcessMessage, packet_type='" << packet.type() << "'\n";
  ...
  gzlog << "ConnectionManager::ProcessMessage finishes\n";
}

//////////////////////////////////////////////////
void ConnectionManager::OnAccept(ConnectionPtr _newConnection)
{
  gzlog << "ConnectionManager::OnAccept started\n";
  using namespace boost::placeholders;
  ...
  gzlog << "ConnectionManager::OnAccept returns\n";
}
...
{
  gzlog << "ConnectionManager::OnRead started\n";
  if (_data.empty())
  {
  ...
  gzlog << "ConnectionManager::OnRead, packet='" << packet.type() << "'\n";
  ...
  gzlog << "ConnectionManager::OnRead returns\n";
}
...
{
  gzlog << "ConnectionManager::Advertise started, msgType='" << msgType << "'\n";
  if (!this->initialized)
    return;
  ...
  gzlog << "ConnectionManager::Advertise returns\n";
}

//////////////////////////////////////////////////
void ConnectionManager::RegisterTopicNamespace(const std::string &_name)
{
  gzlog << "ConnectionManager::RegisterTopicNamespace called, namespace = '"<< _name << "'\n";
  if (!this->initialized)
    return;
  ...
  gzlog << "ConnectionManager::RegisterTopicNamespace returns\n";
}
...
{
  gzlog << "ConnectionManager::GetTopicNamespaces started "<< this->namespaces.size() << " namespaces\n";
  if (!this->initialized)
  {
  ...
  gzlog << "ConnectionManager::GetTopicNamespaces return\n";
}
...
void ConnectionManager::Subscribe(const std::string &_topic,
                                  ...
                                  bool _latching)
{
  gzlog << "ConnectionManager::Subscribe started, topic='" << _topic << "'\n";
  if (!this->initialized)
  {
  ...
  gzlog << "ConnectionManager::Subscribe returns\n";
}
...
ConnectionPtr ConnectionManager::ConnectToRemoteHost(const std::string &_host,
                                                     unsigned int _port)
{
  gzlog << "ConnectionManager::ConnectToRemoteHost started, host='" << _host << "'\n";
  ConnectionPtr conn;
  ...
  gzlog << "ConnectionManager::ConnectToRemoteHost returns\n";
  return conn;
}
