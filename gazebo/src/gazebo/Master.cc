...
{
  gzlog << "Master::Init started, port =" << _port << ", connection=" << this->dataPtr->connection << "\n";
  try
  {
  ...
  gzlog << "Master::Init finished\n";
}

//////////////////////////////////////////////////
void Master::OnAccept(transport::ConnectionPtr _newConnection)
{
  gzlog << "Master::OnAccept started, _newConnection=" << _newConnection << ", pool " << this->dataPtr->connections.size() << "\n";
  ...
  gzlog << "Master::OnAccept, prepare to send namespaces, " << this->dataPtr->worldNames.size() << " worldnames, _newConnection=" << _newConnection << "\n";
  ...
  gzlog << "Master::OnAccept, prepare to send publishers, _newconn=" << _newConnection << "\n";
  ...
  // Add the connection to our list
  {
    ...
    gzlog << "Master::OnAccept prepare to start read on " << index << " , _newconn=" << _newConnection << "\n";
    // Start reading from the connection
    using namespace boost::placeholders;
    _newConnection->AsyncRead(
        boost::bind(&Master::OnRead, this, index, _1));
  }
  gzlog << "Master::OnAccept finishes, _newconn=" << _newConnection << "\n";
}
...
{
  gzlog << "Master::OnRead started, connection=" << _connectionIndex << "\n";
  ...
  if (!_data.empty())
  {
    ...
    gzlog << "Master::OnRead, read msgs from index " << _connectionIndex << ", now " << this->dataPtr->msgs.size() << " messages\n";
  }
  else
  {
...
void Master::ProcessMessage(const unsigned int _connectionIndex,
                            const std::string &_data)
{
  gzlog << "Master::ProcessMessage started\n";
  transport::ConnectionPtr conn = this->dataPtr->connections[_connectionIndex];
  ...
  gzlog << "Master::ProcessMessage, packet '" << packet.type() << "'\n";
  ...
  gzlog << "Master::ProcessMessage finished\n";
}

//////////////////////////////////////////////////
void Master::Run()
{
  gzlog << "Master::Run started\n";
